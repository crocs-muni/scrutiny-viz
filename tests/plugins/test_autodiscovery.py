# scrutiny-viz/tests/plugins/test_autodiscovery.py
...
class ShellBlankMapper(MapperPlugin):
    spec = MapperSpec(name=\"shellblank\", aliases=(\"blankmapper\",), description=\"temporary autodiscovery test mapper\")
    def ingest(self, source_path):
        return {\"ok\": True}
    def map_groups(self, groups, context: MappingContext) -> dict:
        return {\"_type\": \"shellblank\", \"Shell\": [{\"name\": \"ok\", \"value\": True}]}
    def map_source(self, source, context: MappingContext) -> dict:
        return {\"_type\": \"shellblank\", \"Shell\": [{\"name\": \"ok\", \"value\": True}]}
...
class ShellComparator(ComparatorPlugin):
    spec = ComparatorSpec(name=\"shellcompare\", aliases=(\"shellcmp\",), description=\"temporary autodiscovery test comparator\")
    def compare(self, *, section, key_field, show_field, metadata, reference, tested):
        return {\"section\": section, \"counts\": {\"compared\": 0, \"changed\": 0, \"matched\": 0, \"only_ref\": 0, \"only_test\": 0}, \"stats\": {\"compared\": 0, \"changed\": 0, \"matched\": 0, \"only_ref\": 0, \"only_test\": 0}, \"labels\": {}, \"key_labels\": {}, \"diffs\": [], \"artifacts\": {\"source\": \"shell\"}}
...
""".lstrip(), encoding="utf-8")
    ...
    with monkeypatch.context() as m:
        ...
        payload = plugin.compare(section="Shell", key_field="name", show_field="name", metadata={}, reference=[], tested=[])
        assert payload["section"] == "Shell"
        assert payload["artifacts"]["source"] == "shell"


def test_autodiscovery_finds_new_viz(monkeypatch, tmp_path: Path):
    pkg_dir = tmp_path / "fake_report_viz"
    pkg_dir.mkdir()
    (pkg_dir / "__init__.py").write_text("", encoding="utf-8")
    (pkg_dir / "shellviz.py").write_text("""
from dominate import tags
from report.viz.contracts import VizPlugin, VizSpec

class ShellViz(VizPlugin):
    spec = VizSpec(name=\"shellviz\", slot=\"utility\", aliases=(\"sviz\",), description=\"temporary autodiscovery test viz\")
    def render(self, **kwargs):
        return tags.div(\"shellviz-ok\")

PLUGINS = [ShellViz()]
""".lstrip(), encoding="utf-8")
    monkeypatch.syspath_prepend(str(tmp_path))
    with monkeypatch.context() as m:
        m.setattr(viz_registry, "_package_name", lambda: "fake_report_viz")
        viz_registry.discover_builtin_viz(force=True)
        assert "shellviz" in viz_registry.list_types()
        assert viz_registry.normalize_name("sviz") == "shellviz"
        plugin = viz_registry.get_plugin("shellviz")
        html = str(plugin.render(section_name="S", section={}, idx=0, ref_name="r", prof_name="p", variant=None))
        assert "shellviz-ok" in html
