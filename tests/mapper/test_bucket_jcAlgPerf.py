# scrutiny-viz/tests/mapper/test_bucket_jcAlgPerf.py
...
def test_jcAlgPerf_regression(csv_path, tmp_path):
    payload = run_mapper(csv_path, mapper_type="jcperf")
    assert_bucket(BUCKET, payload, csv_path=csv_path)

    schema = find_schema_for_bucket(BUCKET)
    if schema is None:
        pytest.skip("No matching schema found for jcAlgPerf")
